...
export const getAllContests = async (date: string, createdAt?: string, howMany = 10): Promise<AuthApiData> => {
  ...
  return await fetch(`/contest/?deadlineDate=${date}&createdAt=${createdAt}&howMany=${howMany}`, fetchData)
    .then((data) => data.json())
